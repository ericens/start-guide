...
//    //zlx  可以这样 System.setProperty("sun.misc.ProxyGenerator.saveGeneratedFiles","true");      //      里面使用了      result = parseBoolean(System.getProperty(name));
...
//        final byte[] classFile = gen.generateClassFile(); //zlx 正真产生class
//
//        if (saveGeneratedFiles) { // 确认是否需要 print class
//            java.security.AccessController.doPrivileged(
...
//         * methods.
//         */
//        Method[] methodx =Object.class.getDeclaredMethods();
//        for (Class<?> intf : interfaces) {
//            label:
//            for (Method m : intf.getMethods()) {
//                for (int i = 0; i <methodx.length ; i++) {
//                    if( m.getName().equals( methodx[i].getName())){
//                        System.out.println("过滤object 方法" + m.getName());
//                        break label;
//                    }
//                    if(Modifier.isFinal(m.getModifiers())){
//                        System.out.println("过滤final 方法" + m.getName());
//                        break label;
//                    }
//                }
...
//        }
//        /** 第2步  zlx为所有属性和方法 组装FieldInfo MethodInfo对象 **/
//        /* ============================================================
//         * Step 2: Assemble FieldInfo and MethodInfo structs for all of
//         * fields and methods in the class we are generating.
//         */
//        fields.add(new FieldInfo("h",
...
//                    // zlx 组装 private static Method m1; m2.m3.m4..... 属性
...
//        cp.getClass(dotToSlash(interfaces[0].getName()));
...
//            dout.writeShort(cp.getClass(dotToSlash(interfaces[0].getName())));
//                                        // u2 super_class;
...
//                m.write(dout);  // 已经从ProxyMethod 转换到MethodInfo了。
//            }
//
//                                         // u2 attributes_count;
...
//     */
//    private class ProxyMethod { //zlx  代表了一个在proxy class的 proxy method。 会把调用dispatch到 invocation handler 里面
//
...
//         */
//        private MethodInfo generateMethod() throws IOException { //固定流程产生 一个在proxy class的 proxy method
...
//            //zlx equal的方法代码：             return (Boolean)super.h.invoke(this, m1, new Object[]{var1});
//            DataOutputStream out = new DataOutputStream(minfo.code); // zlx 写入字节码
//
//            code_aload(0, out); // aload_0
//
//            out.writeByte(opc_getfield); //获取父类属性h getfield #16 <java/lang/reflect/Proxy.h>
...
//            code_aload(0, out);
//
//            out.writeByte(opc_getstatic); //获取属性m getstatic #20 <com/sun/proxy/$Proxy0.m1
//            out.writeShort(cp.getFieldRef(
//                dotToSlash(className),
//                methodFieldName, "Ljava/lang/reflect/Method;"));
//
//            if (parameterTypes.length > 0) { // 写入参数，这个是动态变化的。
//
//                code_ipush(parameterTypes.length, out); //数组长度入栈 iconst_1
//
//                out.writeByte(opc_anewarray); //anewarray #22 <java/lang/Object>
...
//            }
//
//            out.writeByte(opc_invokeinterface);  //zlx// 调用invoke方法;  invokeinterface #28 <java/lang/reflect/InvocationHandler.invoke
...
//
//                out.writeByte(opc_pop); //pop 返回
//
//                out.writeByte(opc_return); // 返回
//
//            } else {
//                //根据返回值，进行强制类型转换
...
//                out.writeByte(opc_invokespecial);
//        out.writeShort(cp.getMethodRef(
//                dotToSlash(superclassName),  //superclassName, zlx
//            "<init>", "(Ljava/lang/reflect/InvocationHandler;)V"));
//
//        code_aload(0, out);
//        code_aload(1, out);
//
//        out.write(opc_putfield);
//        out.writeShort(cp.getFieldRef(
//                dotToSlash(className),
//                handlerFieldName, "Ljava/lang/reflect/InvocationHandler;"));
//
////        putfield #2 <ProxyMode/ PlanGSdg0.h
//
...
//        MethodInfo minfo = new MethodInfo(
//            "<clinit>", "()V", ACC_STATIC); //zlx 本身也是一个方法
//
//        int localSlot0 = 1;
//        short pc, tryBegin = 0, tryEnd;
//
...
//
//        /** method descriptor for wrapper class "valueOf" zlx.factory method */
